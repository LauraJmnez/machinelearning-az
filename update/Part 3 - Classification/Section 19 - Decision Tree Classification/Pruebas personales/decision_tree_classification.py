# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 12:43:29 2026
...
# Escalado de variableS : LOS ÁRBOLES DE DECISIÓN NO SE BASAN EN LA DISTANCIA EUCLIDEA, ASÍ QUE NO NECESITAMOS ESCALAR NUESTROS DATOS
...
X1, X2 = np.meshgrid(np.arange(start = X_set[:, 0].min() -1, stop = X_set[:, 0]. max() + 1, step = 1), #con meshgrid se pintan muchos puntitos (pixeles) muy pequeños a muy poca distancia
                     np.arange(start = X_set[:, 1].min() -1, stop = X_set[:, 1]. max() + 1, step = 500)) # haciendo que se coloree el fondo
plt.contourf(X1, X2, classifier.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
             alpha = 0.75, cmap = ListedColormap(('red', 'green')))
plt.xlim(X1.min(), X1.max())
plt.ylim(X2.min(), X2.max())
for i, j in enumerate(np.unique(y_test)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('Árbol de decisión (Testing set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show
