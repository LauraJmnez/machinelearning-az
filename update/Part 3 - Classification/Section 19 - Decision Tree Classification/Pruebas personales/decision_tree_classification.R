...
## Escalado de valores
#Los árboles no se basan en la distancia, así que no se escalan
...
y_pred = predict(classifier, newdata = testing_set[,-3], type = "class") #Para que no me calcule la probabilidad
...
plot(set[, -3], main = 'Clasificación con árboles de decisión (Conjunto de Training)',
...
colnames(grid_set) = c('Age', 'EstimatedSalary')
y_grid = predict(classifier, newdata = grid_set, type = "class")
plot(set[, -3], main = 'Clasificación con árboles de decisión (Conjunto de Testing)',
     xlab = 'Edad', ylab = 'Sueldo Estimado', 
     xlim =range(X1), ylim = range(X2))
contour(X1, X2, matrix(as.numeric(y_grid), length(X1), length(X2)), 
        add = TRUE)
